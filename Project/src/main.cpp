...
int main(int argc, char *argv[]) {
 ...
 try
 {
    ...
    hold_timer.async_wait([&](boost::system::error_code ec){ if (! ec) { TIMEOUT to,  send_event(to); }  }); 
    std::thread timer_th([&]{ while(1){if (timer.stopped()) { timer.reset(); }; timer.run();} });

    /* flood loop depending on SM state */
    std::thread flooder_th([&] {  while(1) {  if ( ISIS_ADJ::is_in_state<Up>() ) {
                           std::cout << "flooding LSPs >> DUT" << std::endl;
    ...
    while(1) {
        ...
        hold_timer.async_wait([&](boost::system::error_code ec){ if (! ec) {  TIMEOUT to, send_event(to); }  });
        ISIS_PKT packet;
        std::size_t bytes_copied = buffer_copy(packet.data_.prepare(s.get_data()->size()),s.get_data()->data());
