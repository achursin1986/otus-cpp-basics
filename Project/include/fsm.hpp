...
{
  void entry() override { std::cout << "ISIS Adj is : Down" << std::endl; };
  void react(const ISIS_PKT & e) override; 
};

class Init : public ISIS_ADJ
{
 void entry() override { std::cout << "ISIS Adj is : Init" << std::endl; };
 void react(const ISIS_PKT & e) override;
...
void Down::react(const ISIS_PKT& e) {
 /* have to copy here as FSM provides const buffer and we want to use istream */
  boost::asio::streambuf data_local_;            
  std::size_t bytes_copied = buffer_copy(data_local_.prepare(e.data_.size()),e.data_.data());
 ...
 /*  send hello with Init filling neighbor data */ 
  if ( (hdr_1_r.pdu_type() == p2p_hello) && ( payload_0_r.adjacency_state() == down)  ) {
           #ifdef DEBUG
           ...
           payload_0.ext_neighbor_local_circuit_id(payload_0_r.ext_local_circuit_id());
           hdr_1.pdu_type(p2p_hello);
           hdr_1.length_indicator(20);
           /* length accounts LLC part */
           hdr_0.length(sizeof(hdr_1) + sizeof(hdr_2) +sizeof(payload_0)+sizeof(payload_1)+sizeof(payload_2)+sizeof(payload_3) + 2 );
           hdr_2.pdu_length(sizeof(hdr_1) + sizeof(hdr_2) +sizeof(payload_0)+sizeof(payload_1)+sizeof(payload_2)+sizeof(payload_3));
            
           os << hdr_0 << hdr_1 << hdr_2 << payload_0 << payload_1 << payload_2 << payload_3;
...
void Init::react(const ISIS_PKT & e) {
           ...
           if ( (hdr_1_r.pdu_type() == p2p_hello) && ( payload_0_r.adjacency_state() == init)  && (std::equal(SYS_ID, SYS_ID + sizeof(SYS_ID), payload_0_r.neighbor_sysid())) )  {
                      #ifdef DEBUG
...
void Up::react(const ISIS_PKT & e) {
           ...
           /* CSNP and CSNP support from neighbor is not required for now, will be added later */
          ...
          } else if ( ( hdr_1_r.pdu_type() == l2_lsp ) || ( hdr_1_r.pdu_type() == l2_csnp ) || ( hdr_1_r.pdu_type() == l2_psnp )) {
           /* if CSNP or LSP need to send empty CSNP as we don't store data for hello  */
                 boost::asio::streambuf packet;
                 std::ostream os(&packet);
                 eth_header hdr_0;
                 isis_header hdr_1;
                 ...
                 hdr_0.length(sizeof(hdr_1) + sizeof(hdr_2) +sizeof(payload_0) + 2 );
